...
fn parse() -> (HashMap<&'static str, usize>, Vec<Vec<usize>>) {
    let mut mapping = HashMap::<&str, usize>::new();
    let mut edges = Vec::new();

    let idx = |s, m: &mut HashMap<&str, usize>, e: &mut Vec<Vec<usize>>| {
        *m.entry(s).or_insert_with(|| {
            e.push(Vec::new());
            e.len() - 1
        })
    };

    for (s, ds) in INPUT.lines().skip(1).map(|l| l.split_once(':').unwrap()) {
        let si = idx(s, &mut mapping, &mut edges);
        for d in ds.split_whitespace() {
            let di = idx(d, &mut mapping, &mut edges);
            edges[si].push(di);
        }
    }
...
#[memoize(Ignore: edges, Ignore: banned)]
fn dfs(start: usize, end: usize, edges: &Vec<Vec<usize>>, banned: &Vec<usize>) -> usize {
    if start == end {
        return 1;
    }
...
fn part1() {
    ...
    let total = dfs(mapping["you"], mapping["out"], &edges, &vec![]);
